...
void CoAmp::init(SensorParam_t& sensorParam) {
...
void CoAmp::allocMemory() {
    ...
    m_piEMGSample     = new uint16_t [NUM_EMG_CHANNELS];
    m_piTempEMGSample  = new uint16_t [iWindowLength * NUM_EMG_CHANNELS];

    m_pfEMGSample      = new float [NUM_EMG_CHANNELS];
    m_piBuffer         = new RingBuffer<uint16_t>(iWindowLength, NUM_EMG_CHANNELS, m_sensorParam.iHopLength, RingBuffer<uint16_t>::Replace);
...
//    pInstance->evalTime(emgCANMsg);
    ...
    switch (emgCANMsg->can_id) {
        case 0x451:
            pSample[0] = ((uint16_t)emgCANMsg->data[1] << 8) | emgCANMsg->data[2];
            pSample[1] = ((uint16_t)emgCANMsg->data[3] << 8) | emgCANMsg->data[4];
            pSample[2] = ((uint16_t)emgCANMsg->data[5] << 8) | emgCANMsg->data[6];
            break;
        case 0x452:
            pSample[3] = ((uint16_t)emgCANMsg->data[1] << 8) | emgCANMsg->data[2];
            pSample[4] = ((uint16_t)emgCANMsg->data[3] << 8) | emgCANMsg->data[4];
            pSample[5] = ((uint16_t)emgCANMsg->data[5] << 8) | emgCANMsg->data[6];
            break;
        case 0x453: {
            pSample[6] = ((uint16_t) emgCANMsg->data[1] << 8) | emgCANMsg->data[2];
            pSample[7] = ((uint16_t) emgCANMsg->data[3] << 8) | emgCANMsg->data[4];

            pInstance->m_numSamples++;
...
Error_t CoAmp::getSamples(float *pfSample, size_t iNumSamples) {
    ...
    for (size_t i=0; i<iNumSamples * NUM_EMG_CHANNELS; i++) {
...
void CoAmp::setZero() {
    std::fill(m_piEMGSample, &m_piEMGSample[NUM_EMG_CHANNELS], MAX_15);
    std::fill(m_piTempEMGSample, &m_piTempEMGSample[m_sensorParam.iWindowLength * NUM_EMG_CHANNELS], MAX_15);
}
